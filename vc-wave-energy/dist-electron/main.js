...
path.join(APP_ROOT, "Shared/data/waverider.json");
const USER_DATA_DIR = app.getPath("userData");
const USER_DATA_FILE = path.join(app.getPath("userData"), "waverider.json");
const pacwave_config = JSON.parse(
  fs.readFileSync(PACWAVE_CONFIG, "utf-8")
);
pacwave_config.ipc.uds_path;
pacwave_config.healthcheck.interval_ms;
pacwave_config.healthcheck.max_failures;
const REMOTE_HOST = pacwave_config.remote_host;
const REMOTE_FILE = pacwave_config.remote_file;
...
let send;
let pollTimer = null;
let serialBinding = SerialPort.binding;
function initArduino(sender, binding = SerialPort.binding) {
  send = sender;
  serialBinding = binding;
  pollTimer = setInterval(tryArduinoConnection, 1e3);
...
async function tryArduinoConnection() {
  const ports2 = await serialBinding.list();
  const arduinoPort = ports2.find(
    (p) => p.vendorId && (p.vendorId === FEATHER_VENDOR_ID || p.productId === FEATHER_PRODUCT_ID || p.vendorId === MINIMA_VENDOR_ID || p.productId === MINIMA_PRODUCT_ID)
  );
  if (!arduinoPort) {
    if (port && port.isOpen) {
      port.close();
...
const debugFactory = /* @__PURE__ */ getDefaultExportFromCjs(srcExports);
...
const MockBinding = {
  ...
  async list() {
    debug(null, "list");
    return Object.values(ports$1).map((port2) => port2.info);
  },
  async open(options) {
    var _a2;
    ...
    if ((_a2 = port2.openOpt) === null || _a2 === void 0 ? void 0 : _a2.lock) {
...
class MockPortBinding {
    ...
    if (this.port.data.length <= 0) {
      return new Promise((resolve, reject) => {
        this.pendingRead = (err) => {
          ...
          this.read(buffer2, offset, length).then(resolve, reject);
        };
      });
    }
  ...
  async write(buffer2) {
    ...
    debug(this.serialNumber, "write", buffer2.length, "bytes");
  ...
  async getBaudRate() {
    ...
    await resolveNextTick();
    if (!((_a2 = this.port.openOpt) === null || _a2 === void 0 ? void 0 : _a2.baudRate)) {
...
const RESPONSE = '{"channel":"SOT","mssg":"","data":0}{"channel":"WAVEDATA","mssg":"","data":1}{"channel":"WAVEDATA","mssg":"","data":2}{"channel":"WAVEDATA","mssg":"","data":3}{"channel":"EOT","mssg":"","data":0}';
function createMockArduinoBinding() {
  MockBinding.reset();
  ...
  return {
    ...MockBinding,
    async open(options) {
      const port2 = await MockBinding.open(options);
      const write = port2.write.bind(port2);
      port2.write = async (buffer2) => {
        await write(buffer2);
        setTimeout(() => {
          if (port2.isOpen) {
            port2.emitData(RESPONSE);
          }
        }, 0);
      };
      return port2;
...
app.whenReady().then(() => {
  createWindow();
  const arduinoBinding = process.env.WAVE_ENERGY_MOCK_ARDUINO === "1" ? createMockArduinoBinding() : void 0;
  initArduino(safeSend, arduinoBinding);
  registerArduinoHandlers();
  refreshData();
  setInterval(refreshData, 1e3 * 60 * 60);
