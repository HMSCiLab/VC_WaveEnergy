import { app as q, ipcMain as le, BrowserWindow as gu } from "electron";
import J from "node:path";
import { fileURLToPath as on } from "node:url";
import Ve from "node:fs";
import { createRequire as sn } from "node:module";
import { spawn as cn } from "child_process";
import Bu from "fs";
import an from "path";
const Dn = on(import.meta.url), yu = J.dirname(Dn), ln = yu, pe = J.join(yu, ".."), Le = J.join(pe, "config"), fn = J.join(Le, "pacwave.config.json"), dn = J.join(Le, "arduino.config.json"), pn = J.join(Le, "customwave.config.json");
J.join(pe, "Shared/data/waverider.json");
const hn = q.getPath("userData"), vu = J.join(q.getPath("userData"), "waverider.json"), he = JSON.parse(
  Ve.readFileSync(fn, "utf-8")
);
he.ipc.uds_path;
he.healthcheck.interval_ms;
he.healthcheck.max_failures;
const Cn = he.remote_host, Fn = he.remote_file, Ce = JSON.parse(
  Ve.readFileSync(dn, "utf-8")
), En = Ce.ports.featherM0_vendor_id, An = Ce.ports.featherM0_wifi_product_id, mn = Ce.ports.r4minima_vendor_id, _n = Ce.ports.r4minima_product_id, gn = Ce.baud_rate, wu = JSON.parse(
  Ve.readFileSync(pn, "utf-8")
), Bn = wu.height_selection_options, yn = wu.period_selection_options;
var vn = /[\u1680\u2000-\u200A\u202F\u205F\u3000]/, wn = /[\xAA\xB5\xBA\xC0-\xD6\xD8-\xF6\xF8-\u02C1\u02C6-\u02D1\u02E0-\u02E4\u02EC\u02EE\u0370-\u0374\u0376\u0377\u037A-\u037D\u037F\u0386\u0388-\u038A\u038C\u038E-\u03A1\u03A3-\u03F5\u03F7-\u0481\u048A-\u052F\u0531-\u0556\u0559\u0561-\u0587\u05D0-\u05EA\u05F0-\u05F2\u0620-\u064A\u066E\u066F\u0671-\u06D3\u06D5\u06E5\u06E6\u06EE\u06EF\u06FA-\u06FC\u06FF\u0710\u0712-\u072F\u074D-\u07A5\u07B1\u07CA-\u07EA\u07F4\u07F5\u07FA\u0800-\u0815\u081A\u0824\u0828\u0840-\u0858\u0860-\u086A\u08A0-\u08B4\u08B6-\u08BD\u0904-\u0939\u093D\u0950\u0958-\u0961\u0971-\u0980\u0985-\u098C\u098F\u0990\u0993-\u09A8\u09AA-\u09B0\u09B2\u09B6-\u09B9\u09BD\u09CE\u09DC\u09DD\u09DF-\u09E1\u09F0\u09F1\u09FC\u0A05-\u0A0A\u0A0F\u0A10\u0A13-\u0A28\u0A2A-\u0A30\u0A32\u0A33\u0A35\u0A36\u0A38\u0A39\u0A59-\u0A5C\u0A5E\u0A72-\u0A74\u0A85-\u0A8D\u0A8F-\u0A91\u0A93-\u0AA8\u0AAA-\u0AB0\u0AB2\u0AB3\u0AB5-\u0AB9\u0ABD\u0AD0\u0AE0\u0AE1\u0AF9\u0B05-\u0B0C\u0B0F\u0B10\u0B13-\u0B28\u0B2A-\u0B30\u0B32\u0B33\u0B35-\u0B39\u0B3D\u0B5C\u0B5D\u0B5F-\u0B61\u0B71\u0B83\u0B85-\u0B8A\u0B8E-\u0B90\u0B92-\u0B95\u0B99\u0B9A\u0B9C\u0B9E\
-\uDE6C\uDE75\uDE84\uDE9B-\uDE9F\uDEA1-\uDEAF]|\uD838[\uDC00-\uDC06\uDC08-\uDC18\uDC1B-\uDC21\uDC23\uDC24\uDC26-\uDC2A]|\uD83A[\uDC00-\uDCC4\uDCD0-\uDCD6\uDD00-\uDD4A\uDD50-\uDD59]|\uD83B[\uDE00-\uDE03\uDE05-\uDE1F\uDE21\uDE22\uDE24\uDE27\uDE29-\uDE32\uDE34-\uDE37\uDE39\uDE3B\uDE42\uDE47\uDE49\uDE4B\uDE4D-\uDE4F\uDE51\uDE52\uDE54\uDE57\uDE59\uDE5B\uDE5D\uDE5F\uDE61\uDE62\uDE64\uDE67-\uDE6A\uDE6C-\uDE72\uDE74-\uDE77\uDE79-\uDE7C\uDE7E\uDE80-\uDE89\uDE8B-\uDE9B\uDEA1-\uDEA3\uDEA5-\uDEA9\uDEAB-\uDEBB]|\uD869[\uDC00-\uDED6\uDF00-\uDFFF]|\uD86D[\uDC00-\uDF34\uDF40-\uDFFF]|\uD86E[\uDC00-\uDC1D\uDC20-\uDFFF]|\uD873[\uDC00-\uDEA1\uDEB0-\uDFFF]|\uD87A[\uDC00-\uDFE0]|\uD87E[\uDC00-\uDE1D]|\uDB40[\uDD00-\uDDEF]/, Oe = {
  Space_Separator: vn,
  ID_Start: wn,
  ID_Continue: bn
}, O = {
...
let Te, R, G, me, H, L, I, Ue, De;
var zn = function(u, n) {
  Te = String(u), R = "start", G = [], me = 0, H = 1, L = 0, I = void 0, Ue = void 0, De = void 0;
  do
    I = Sn(), $n[R]();
  while (I.type !== "eof");
  return typeof n == "function" ? Ze({ "": De }, "", n) : De;
};
function Ze(e, u, n) {
  const t = e[u];
...
function W() {
  if (Te[me])
    return String.fromCodePoint(Te.codePointAt(me));
}
function a() {
  const e = W();
  return e === `
` ? (H++, L = 0) : e ? L += e.length : L++, e && (me += e.length), e;
}
const bu = {
  default() {
    switch (g) {
...
`:
  ...
  value() {
    switch (g) {
      ...
      case "n":
        return a(), X("ull"), z("null", null);
      case "t":
        return a(), X("rue"), z("boolean", !0);
      case "f":
        return a(), X("alse"), z("boolean", !1);
      case "-":
      case "+":
        a() === "-" && (U = -1), m = "sign";
        return;
      ...
      case "9":
        ...
        return;
      case "I":
        return a(), X("nfinity"), z("numeric", 1 / 0);
      case "N":
        return a(), X("aN"), z("numeric", NaN);
      case '"':
      case "'":
        ce = a() === '"', E = "", m = "string";
        return;
  ...
  sign() {
    switch (g) {
      ...
      case "9":
        ...
        return;
      case "I":
        return a(), X("nfinity"), z("numeric", U * (1 / 0));
      case "N":
        return a(), X("aN"), z("numeric", NaN);
    }
    throw S(a());
...
const $n = {
  ...
  beforePropertyName() {
    switch (I.type) {
      case "identifier":
      case "string":
        Ue = I.value, R = "afterPropertyName";
        return;
      case "punctuator":
        Fe();
        return;
...
function $e() {
  ...
  if (De === void 0)
    ...
    Array.isArray(u) ? u.push(e) : Object.defineProperty(u, Ue, {
      value: e,
      writable: !0,
      enumerable: !0,
      configurable: !0
...
function S(e) {
  return _e(e === void 0 ? `JSON5: invalid end of input at ${H}:${L}` : `JSON5: invalid character '${zu(e)}' at ${H}:${L}`);
}
function Q() {
  return _e(`JSON5: invalid end of input at ${H}:${L}`);
}
function Qe() {
  return L -= 5, _e(`JSON5: invalid identifier character at ${H}:${L}`);
}
function Nn(e) {
  console.warn(`JSON5: '${zu(e)}' in strings is not valid ECMAScript; consider escaping`);
}
function zu(e) {
  const u = {
    "'": "\\'",
...
function _e(e) {
  ...
  return u.lineNumber = H, u.columnNumber = L, u;
}
  ...
  function f(p) {
    ...
    for (let B = 0; B < p.length; B++) {
      ...
      if (y < " ") {
        ...
        _ += "\\x" + ("00" + P).substring(P.length);
        continue;
      }
      _ += y;
    }
    const k = D || Object.keys(A).reduce((B, y) => A[B] < A[y] ? B : y);
    return _ = _.replace(new RegExp(k, "g"), h[k]), k + _ + k;
  }
  function F(p) {
    if (r.indexOf(p) >= 0)
...
var Tn = In;
const Su = sn(import.meta.url), { SerialPort: eu } = Su("serialport"), { ReadlineParser: Zn } = Su("@serialport/parser-readline");
let T = null, Ne = [], uu = null, ee;
function jn(e) {
  ee = e, setInterval(xn, 1e3);
}
function Rn(e) {
  return e += "}", Tn.parse(e);
}
async function xn() {
  const u = (await eu.list()).find(
    (n) => n.vendorId && (n.vendorId === En || n.productId === An || n.vendorId === mn || n.productId === _n)
  );
  if (!u) {
    T && T.isOpen && (T.close(), T = null, ee("main.ts >> Arduino not connected."));
    return;
  }
  T && T.isOpen || (T = new eu({
    path: u.path,
    ...
    console.log("main.ts >> Arduino connected", u.path), ee("arduino-connected");
  }), T.on("close", () => {
    console.log("main.ts >> Arduino disconnected"), T = null, ee("arduino-disconnected");
  }), T.on("error", (n) => {
    console.log("main.ts >> Arduino not connected."), ee("arduino-error", n.message);
  }), uu = T.pipe(new Zn({ delimiter: "}" })), uu.on(
    "data",
    (n) => {
      const t = Rn(n);
      switch (t.channel) {
...
Data: ${t.data}`);
          break;
        case "EOT":
          ee("complete-wave", Ne), Ne = [];
          break;
        case "WAVEDATA":
          Ne.push(t.data), ee("wave-val", t.data);
          break;
      }
    }
  ));
...
function Jn() {
  le.handle("arduino-status", () => ({ connected: !!(T && T.isOpen) })), le.handle("send-wave", async (e) => {
    const u = JSON.stringify(e);
    return T.write(u + `
`, (n) => {
      console.log(n ? `main.ts >> Error sending command to arduino: ${n}` : `main.ts >> Sent command to arduino: ${u}`);
    }), "OK";
  });
}
function d(e, u, n) {
...
const nu = Symbol("evaluating");
function w(e, u, n) {
  let t;
  Object.defineProperty(e, u, {
    get() {
      if (t !== nu)
        return t === void 0 && (t = nu, t = n()), t;
    },
    set(r) {
      Object.defineProperty(e, u, {
        value: r
...
const Nu = "captureStackTrace" in Error ? Error.captureStackTrace : (...e) => {
};
function ge(e) {
...
function ne(e, u, n) {
  ...
  if (!e.message) {
    const l = Ee((i = (o = (r = e.inst) == null ? void 0 : r._zod.def) == null ? void 0 : o.error) == null ? void 0 : i.call(o, e)) ?? Ee((s = u == null ? void 0 : u.error) == null ? void 0 : s.call(u, e)) ?? Ee((c = n.customError) == null ? void 0 : c.call(n, e)) ?? Ee((D = n.localeError) == null ? void 0 : D.call(n, e)) ?? "Invalid input";
    t.message = l;
  }
...
const Tu = (e, u) => {
  e.name = "$ZodError", Object.defineProperty(e, "_zod", {
    ...
    value: u,
    enumerable: !1
  }), e.message = JSON.stringify(u, Re, 2), Object.defineProperty(e, "toString", {
    value: () => e.message,
    enumerable: !1
  });
}, Zu = d("$ZodError", Tu), ju = d("$ZodError", Tu, { Parent: Error });
...
const Ye = (e) => (u, n, t, r) => {
  ...
  if (i.issues.length) {
    ...
    throw Nu(s, r == null ? void 0 : r.callee), s;
  }
  return i.value;
}, Xe = (e) => async (u, n, t, r) => {
  ...
  return o.issues.length ? {
    success: !1,
    error: new (e ?? Zu)(o.issues.map((i) => ne(i, r, ue())))
  } : { success: !0, data: o.value };
}, tt = /* @__PURE__ */ be(ju), ze = (e) => async (u, n, t) => {
...
}, rt = /* @__PURE__ */ ze(ju), ot = (e) => (u, n, t) => {
  const r = t ? Object.assign(t, { direction: "backward" }) : { direction: "backward" };
  return Ye(e)(u, n, r);
}, it = (e) => (u, n, t) => Ye(e)(u, n, t), st = (e) => async (u, n, t) => {
  const r = t ? Object.assign(t, { direction: "backward" }) : { direction: "backward" };
  return Xe(e)(u, n, r);
}, ct = (e) => async (u, n, t) => Xe(e)(u, n, t), at = (e) => (u, n, t) => {
  const r = t ? Object.assign(t, { direction: "backward" }) : { direction: "backward" };
  return be(e)(u, n, r);
}, Dt = (e) => (u, n, t) => be(e)(u, n, t), lt = (e) => async (u, n, t) => {
  const r = t ? Object.assign(t, { direction: "backward" }) : { direction: "backward" };
  return ze(e)(u, n, r);
}, ft = (e) => async (u, n, t) => ze(e)(u, n, t), dt = "(?:(?:\\d\\d[2468][048]|\\d\\d[13579][26]|\\d\\d0[48]|[02468][048]00|[13579][26]00)-02-29|\\d{4}-(?:(?:0[13578]|1[02])-(?:0[1-9]|[12]\\d|3[01])|(?:0[469]|11)-(?:0[1-9]|[12]\\d|30)|(?:02)-(?:0[1-9]|1\\d|2[0-8])))";
function pt(e) {
...
const Ct = (e) => {
  ...
  bigint: "bigint",
  object: "date"
}, xu = /* @__PURE__ */ d("$ZodCheckLessThan", (e, u) => {
  x.init(e, u);
  const n = Ru[typeof u.value];
...
}), Ju = /* @__PURE__ */ d("$ZodCheckGreaterThan", (e, u) => {
  x.init(e, u);
  const n = Ru[typeof u.value];
  e._zod.onattach.push((t) => {
...
}), _t = /* @__PURE__ */ d("$ZodCheckMultipleOf", (e, u) => {
...
}), gt = /* @__PURE__ */ d("$ZodCheckNumberFormat", (e, u) => {
  var i;
...
}), Se = /* @__PURE__ */ d("$ZodCheckStringFormat", (e, u) => {
  var n, t;
  ...
  u.pattern ?? (u.pattern = At), Se.init(e, u);
}), zt = /* @__PURE__ */ d("$ZodCheckUpperCase", (e, u) => {
  u.pattern ?? (u.pattern = mt), Se.init(e, u);
}), St = /* @__PURE__ */ d("$ZodCheckIncludes", (e, u) => {
  x.init(e, u);
  const n = we(u.includes), t = new RegExp(typeof u.position == "number" ? `^.{${u.position}}${n}` : n);
  ...
  x.init(e, u);
  const n = new RegExp(`^${we(u.prefix)}.*`);
  ...
  x.init(e, u);
  const n = new RegExp(`.*${we(u.suffix)}$`);
  u.pattern ?? (u.pattern = n), e._zod.onattach.push((t) => {
    const r = t._zod.bag;
    r.patterns ?? (r.patterns = /* @__PURE__ */ new Set()), r.patterns.add(n);
  }), e._zod.check = (t) => {
      ...
      for (const F of c) {
        ...
        if (f || v instanceof Promise)
          f = (f ?? Promise.resolve()).then(async () => {
            await v, s.issues.length !== C && (l || (l = oe(s, C)));
          });
        else {
...
}), Vu = /* @__PURE__ */ d("$ZodStringFormat", (e, u) => {
  Se.init(e, u), Mu.init(e, u);
}), It = /* @__PURE__ */ d("$ZodISODateTime", (e, u) => {
  u.pattern ?? (u.pattern = ht(u)), Vu.init(e, u);
}), Lu = /* @__PURE__ */ d("$ZodNumber", (e, u) => {
...
function ru(e, u, n) {
  e.issues.length && u.issues.push(...Iu(n, e.issues)), u.value[n] = e.value;
}
const Rt = /* @__PURE__ */ d("$ZodArray", (e, u) => {
  $.init(e, u), e._zod.parse = (n, t) => {
    ...
    for (let i = 0; i < r.length; i++) {
      ...
      c instanceof Promise ? o.push(c.then((D) => ru(D, n, i))) : ru(c, n, i);
    }
    return o.length ? Promise.all(o).then(() => n) : n;
  };
});
function Be(e, u, n, t, r) {
  if (e.issues.length) {
    if (r && !(n in t))
      return;
    u.issues.push(...Iu(n, e.issues));
  }
...
function Gu(e, u, n, t, r, o) {
  ...
  for (const f in u) {
    ...
    F instanceof Promise ? e.push(F.then((C) => Be(C, n, f, u, l))) : Be(F, n, f, u, l);
  }
...
const xt = /* @__PURE__ */ d("$ZodObject", (e, u) => {
  ...
  e._zod.parse = (s, c) => {
    ...
    for (const F of i.keys) {
      ...
      p instanceof Promise ? l.push(p.then((A) => Be(A, s, F, D, v))) : Be(p, s, F, D, v);
    }
    return o ? Gu(l, D, s, c, t.value, e) : l.length ? Promise.all(l).then(() => s) : s;
  };
}), Jt = /* @__PURE__ */ d("$ZodObjectJIT", (e, u) => {
  xt.init(e, u);
  const n = e._zod.parse, t = We(() => Uu(u)), r = (F) => {
    var k;
    ...
    for (const B of v.keys) {
      const y = A[B], P = tu(B), ke = F[B], rn = ((k = ke == null ? void 0 : ke._zod) == null ? void 0 : k.optout) === "optional";
      C.write(`const ${y} = ${p(B)};`), rn ? C.write(`
        if (${y}.issues.length) {
          if (${P} in input) {
  ...
  let o;
  const i = ge, s = !Ou.jitless, D = s && Ln.value, l = u.catchall;
  let f;
  e._zod.parse = (F, C) => {
    f ?? (f = t.value);
    const v = F.value;
    return i(v) ? s && D && (C == null ? void 0 : C.async) === !1 && C.jitless !== !0 ? (o || (o = r(u.shape)), F = o(F, C), l ? Gu([], v, F, C, f, e) : F) : n(F, C) : (F.issues.push({
      expected: "object",
      code: "invalid_type",
      input: v,
      inst: e
...
const Mt = /* @__PURE__ */ d("$ZodUnion", (e, u) => {
  $.init(e, u), w(e._zod, "optin", () => u.options.some((r) => r._zod.optin === "optional") ? "optional" : void 0), w(e._zod, "optout", () => u.options.some((r) => r._zod.optout === "optional") ? "optional" : void 0), w(e._zod, "values", () => {
    ...
    if (u.options.every((r) => r._zod.pattern)) {
      ...
      return new RegExp(`^(${r.map((o) => He(o.source)).join("|")})$`);
    }
  });
  ...
  e._zod.parse = (r, o) => {
    ...
    return i ? Promise.all(s).then((c) => ou(c, r, e, o)) : ou(s, r, e, o);
  };
}), Vt = /* @__PURE__ */ d("$ZodIntersection", (e, u) => {
  $.init(e, u), e._zod.parse = (n, t) => {
    const r = n.value, o = u.left._zod.run({ value: r, issues: [] }, t), i = u.right._zod.run({ value: r, issues: [] }, t);
    return o instanceof Promise || i instanceof Promise ? Promise.all([o, i]).then(([c, D]) => iu(n, c, D)) : iu(n, o, i);
  };
});
function xe(e, u) {
  if (e === u)
...
function iu(e, u, n) {
  ...
  if (o.length && r && e.issues.push({ ...r, keys: o }), oe(e))
    return e;
  const i = xe(u.value, n.value);
  if (!i.valid)
...
const Lt = /* @__PURE__ */ d("$ZodEnum", (e, u) => {
  $.init(e, u);
  const n = $u(u.entries), t = new Set(n);
  e._zod.values = t, e._zod.pattern = new RegExp(`^(${n.filter((r) => Un.has(typeof r)).map((r) => typeof r == "string" ? we(r) : r.toString()).join("|")})$`), e._zod.parse = (r, o) => {
    const i = r.value;
  ...
  $.init(e, u), e._zod.parse = (n, t) => {
    if (t.direction === "backward")
      throw new ku(e.constructor.name);
...
const Wu = /* @__PURE__ */ d("$ZodOptional", (e, u) => {
  $.init(e, u), e._zod.optin = "optional", e._zod.optout = "optional", w(e._zod, "values", () => u.innerType._zod.values ? /* @__PURE__ */ new Set([...u.innerType._zod.values, void 0]) : void 0), w(e._zod, "pattern", () => {
    ...
    if (u.innerType._zod.optin === "optional") {
      ...
      return r instanceof Promise ? r.then((o) => su(o, n.value)) : su(r, n.value);
    }
    return n.value === void 0 ? n : u.innerType._zod.run(n, t);
  };
}), Gt = /* @__PURE__ */ d("$ZodExactOptional", (e, u) => {
  Wu.init(e, u), w(e._zod, "values", () => u.innerType._zod.values), w(e._zod, "pattern", () => u.innerType._zod.pattern), e._zod.parse = (n, t) => u.innerType._zod.run(n, t);
  ...
  $.init(e, u), e._zod.optin = "optional", w(e._zod, "values", () => u.innerType._zod.values), e._zod.parse = (n, t) => {
    ...
    const r = u.innerType._zod.run(n, t);
    return r instanceof Promise ? r.then((o) => cu(o, u)) : cu(r, u);
  };
});
function cu(e, u) {
...
const Ht = /* @__PURE__ */ d("$ZodPrefault", (e, u) => {
  ...
  $.init(e, u), w(e._zod, "values", () => {
    ...
    const r = u.innerType._zod.run(n, t);
    return r instanceof Promise ? r.then((o) => au(o, e)) : au(r, e);
  };
});
function au(e, u) {
...
const Yt = /* @__PURE__ */ d("$ZodCatch", (e, u) => {
  ...
  $.init(e, u), w(e._zod, "values", () => u.in._zod.values), w(e._zod, "optin", () => u.in._zod.optin), w(e._zod, "optout", () => u.out._zod.optout), w(e._zod, "propValues", () => u.in._zod.propValues), e._zod.parse = (n, t) => {
    if (t.direction === "backward") {
      ...
      return o instanceof Promise ? o.then((i) => Ae(i, u.in, t)) : Ae(o, u.in, t);
    }
    const r = u.in._zod.run(n, t);
    return r instanceof Promise ? r.then((o) => Ae(o, u.out, t)) : Ae(r, u.out, t);
  };
});
function Ae(e, u, n) {
...
function lu(e, u, n, t) {
  if (!e) {
    ...
    t._zod.def.params && (r.params = t._zod.def.params), u.issues.push(de(r));
  }
}
var fu;
class ur {
...
(fu = globalThis).__zod_globalRegistry ?? (fu.__zod_globalRegistry = nr());
...
// @__NO_SIDE_EFFECTS__
function pr(e) {
  return /* @__PURE__ */ se((u) => u.normalize(e));
}
// @__NO_SIDE_EFFECTS__
function hr() {
...
// @__NO_SIDE_EFFECTS__
function _r(e) {
  const u = /* @__PURE__ */ gr((n) => (n.addIssue = (t) => {
    if (typeof t == "string")
      n.issues.push(de(t, n.value, u._zod.def));
    else {
      const r = t;
      r.fatal && (r.continue = !1), r.code ?? (r.code = "custom"), r.input ?? (r.input = n.value), r.inst ?? (r.inst = u), r.continue ?? (r.continue = !u._zod.def.abort), n.issues.push(de(r));
    }
  }, e(n.value, n)));
  return u;
}
...
function Yu(e, u) {
  ...
  const r = (l) => {
    ...
    if (e.external) {
      ...
      return l[1].defId = _, { defId: _, ref: `${h("__shared")}#/${f}/${_}` };
    }
    if (l[1] === n)
      return { ref: "#" };
  ...
  const t = (D) => {
    ...
    if (l.ref = null, C) {
      ...
      if (A.$ref && (e.target === "draft-07" || e.target === "draft-04" || e.target === "openapi-3.0") ? (f.allOf = f.allOf ?? [], f.allOf.push(A)) : Object.assign(f, A), Object.assign(f, F), D._zod.parent === C)
        for (const _ in f)
          _ === "$ref" || _ === "allOf" || _ in F || delete f[_];
      if (A.$ref && p.def)
        for (const _ in f)
  ...
  try {
    ...
    return Object.defineProperty(D, "~standard", {
      value: {
        ...u["~standard"],
        jsonSchema: {
          input: ve(u, "input", e.processors),
          output: ve(u, "output", e.processors)
        }
      },
      enumerable: !1,
      writable: !1
...
const Br = (e, u = {}) => (n) => {
  const t = Ku({ ...n, processors: u });
  return Z(e, t), Yu(t, e), Xu(t, e);
}, ve = (e, u, n = {}) => (t) => {
  const { libraryOptions: r, target: o } = t ?? {}, i = Ku({ ...r ?? {}, target: o, io: u, processors: n });
  return Z(e, i), Yu(i, e), Xu(i, e);
}, yr = {
  guid: "uuid",
  url: "uri",
  datetime: "date-time",
  ...
  const r = e._zod.def, o = $u(r.entries);
...
const Ur = (e, u) => {
  Zu.init(e, u), e.name = "ZodError", Object.defineProperties(e, {
    format: {
      value: (n) => nt(e, n)
      // enumerable: false,
    },
  ...
  Parent: Error
}), Gr = /* @__PURE__ */ Ye(M), Wr = /* @__PURE__ */ Xe(M), qr = /* @__PURE__ */ be(M), Hr = /* @__PURE__ */ ze(M), Kr = /* @__PURE__ */ ot(M), Yr = /* @__PURE__ */ it(M), Xr = /* @__PURE__ */ st(M), Qr = /* @__PURE__ */ ct(M), eo = /* @__PURE__ */ at(M), uo = /* @__PURE__ */ Dt(M), no = /* @__PURE__ */ lt(M), to = /* @__PURE__ */ ft(M), N = /* @__PURE__ */ d("ZodType", (e, u) => ($.init(e, u), Object.assign(e["~standard"], {
  jsonSchema: {
    input: ve(e, "input"),
    output: ve(e, "output")
  }
}), e.toJSONSchema = Br(e, {}), e.def = u, e.type = u.type, Object.defineProperty(e, "_def", { value: u }), e.check = (...n) => e.clone(K(u, {
  checks: [
    ...u.checks ?? [],
  ...
  parent: !0
}), e.with = e.check, e.clone = (n, t) => Y(e, n, t), e.brand = () => e, e.register = (n, t) => (n.add(e, t), e), e.parse = (n, t) => Gr(e, n, t, { callee: e.parse }), e.safeParse = (n, t) => qr(e, n, t), e.parseAsync = async (n, t) => Wr(e, n, t, { callee: e.parseAsync }), e.safeParseAsync = async (n, t) => Hr(e, n, t), e.spa = e.safeParseAsync, e.encode = (n, t) => Kr(e, n, t), e.decode = (n, t) => Yr(e, n, t), e.encodeAsync = async (n, t) => Xr(e, n, t), e.decodeAsync = async (n, t) => Qr(e, n, t), e.safeEncode = (n, t) => eo(e, n, t), e.safeDecode = (n, t) => uo(e, n, t), e.safeEncodeAsync = async (n, t) => no(e, n, t), e.safeDecodeAsync = async (n, t) => to(e, n, t), e.refine = (n, t) => e.check(To(n, t)), e.superRefine = (n) => e.check(Zo(n)), e.overwrite = (n) => e.check(/* @__PURE__ */ se(n)), e.optional = () => Eu(e), e.exactOptional = () => Bo(e), e.nullable = () => Au(e), e.nullish = () => Eu(Au(e)), e.nonoptional = (n) => So(e, n), e.array = () => lo(e), e.or = (n) => Co([e, n]), e.and = (n) => Eo(e, n), e.transform = (n) => mu(e, _o(n)), e.default = (n) => wo(e, n), e.prefault = (n) => zo(e, n), e.catch = (n) => Oo(e, n), e.pipe = (n) => mu(e, n), e.readonly = () => Po(e), e.describe = (n) => {
  const t = e.clone();
  return ae.add(t, { description: n }), t;
}, Object.defineProperty(e, "description", {
  get() {
    var n;
    return (n = ae.get(e)) == null ? void 0 : n.description;
  },
  configurable: !0
}), e.meta = (...n) => {
  if (n.length === 0)
  ...
  Mu.init(e, u), N.init(e, u), e._zod.processJSONSchema = (t, r, o) => vr(e, t, r);
  const n = e._zod.bag;
  e.format = n.format ?? null, e.minLength = n.minimum ?? null, e.maxLength = n.maximum ?? null, e.regex = (...t) => e.check(/* @__PURE__ */ cr(...t)), e.includes = (...t) => e.check(/* @__PURE__ */ lr(...t)), e.startsWith = (...t) => e.check(/* @__PURE__ */ fr(...t)), e.endsWith = (...t) => e.check(/* @__PURE__ */ dr(...t)), e.min = (...t) => e.check(/* @__PURE__ */ ye(...t)), e.max = (...t) => e.check(/* @__PURE__ */ qu(...t)), e.length = (...t) => e.check(/* @__PURE__ */ Hu(...t)), e.nonempty = (...t) => e.check(/* @__PURE__ */ ye(1, ...t)), e.lowercase = (t) => e.check(/* @__PURE__ */ ar(t)), e.uppercase = (t) => e.check(/* @__PURE__ */ Dr(t)), e.trim = () => e.check(/* @__PURE__ */ hr()), e.normalize = (...t) => e.check(/* @__PURE__ */ pr(...t)), e.toLowerCase = () => e.check(/* @__PURE__ */ Cr()), e.toUpperCase = () => e.check(/* @__PURE__ */ Fr()), e.slugify = () => e.check(/* @__PURE__ */ Er());
}), oo = /* @__PURE__ */ d("ZodStringFormat", (e, u) => {
  Vu.init(e, u), ro.init(e, u);
}), en = /* @__PURE__ */ d("ZodNumber", (e, u) => {
  Lu.init(e, u), N.init(e, u), e._zod.processJSONSchema = (t, r, o) => wr(e, t, r), e.gt = (t, r) => e.check(/* @__PURE__ */ pu(t, r)), e.gte = (t, r) => e.check(/* @__PURE__ */ Ie(t, r)), e.min = (t, r) => e.check(/* @__PURE__ */ Ie(t, r)), e.lt = (t, r) => e.check(/* @__PURE__ */ du(t, r)), e.lte = (t, r) => e.check(/* @__PURE__ */ Pe(t, r)), e.max = (t, r) => e.check(/* @__PURE__ */ Pe(t, r)), e.int = (t) => e.check(Cu(t)), e.safe = (t) => e.check(Cu(t)), e.positive = (t) => e.check(/* @__PURE__ */ pu(0, t)), e.nonnegative = (t) => e.check(/* @__PURE__ */ Ie(0, t)), e.negative = (t) => e.check(/* @__PURE__ */ du(0, t)), e.nonpositive = (t) => e.check(/* @__PURE__ */ Pe(0, t)), e.multipleOf = (t, r) => e.check(/* @__PURE__ */ hu(t, r)), e.step = (t, r) => e.check(/* @__PURE__ */ hu(t, r)), e.finite = () => e;
  const n = e._zod.bag;
  e.minValue = Math.max(n.minimum ?? Number.NEGATIVE_INFINITY, n.exclusiveMinimum ?? Number.NEGATIVE_INFINITY) ?? null, e.maxValue = Math.min(n.maximum ?? Number.POSITIVE_INFINITY, n.exclusiveMaximum ?? Number.POSITIVE_INFINITY) ?? null, e.isInt = (n.format ?? "").includes("int") || Number.isSafeInteger(n.multipleOf ?? 0.5), e.isFinite = !0, e.format = n.format ?? null;
}), io = /* @__PURE__ */ d("ZodNumberFormat", (e, u) => {
  Tt.init(e, u), en.init(e, u);
});
function Cu(e) {
...
const Do = /* @__PURE__ */ d("ZodArray", (e, u) => {
  Rt.init(e, u), N.init(e, u), e._zod.processJSONSchema = (n, t, r) => $r(e, n, t, r), e.element = u.element, e.min = (n, t) => e.check(/* @__PURE__ */ ye(n, t)), e.nonempty = (n) => e.check(/* @__PURE__ */ ye(1, n)), e.max = (n, t) => e.check(/* @__PURE__ */ qu(n, t)), e.length = (n, t) => e.check(/* @__PURE__ */ Hu(n, t)), e.unwrap = () => e.element;
});
function lo(e, u) {
  return /* @__PURE__ */ Ar(Do, e, u);
}
const fo = /* @__PURE__ */ d("ZodObject", (e, u) => {
  Jt.init(e, u), N.init(e, u), e._zod.processJSONSchema = (n, t, r) => Nr(e, n, t, r), w(e, "shape", () => u.shape), e.keyof = () => Ao(Object.keys(e._zod.def.shape)), e.catchall = (n) => e.clone({ ...e._zod.def, catchall: n }), e.passthrough = () => e.clone({ ...e._zod.def, catchall: Fu() }), e.loose = () => e.clone({ ...e._zod.def, catchall: Fu() }), e.strict = () => e.clone({ ...e._zod.def, catchall: ao() }), e.strip = () => e.clone({ ...e._zod.def, catchall: void 0 }), e.extend = (n) => Kn(e, n), e.safeExtend = (n) => Yn(e, n), e.merge = (n) => Xn(e, n), e.pick = (n) => qn(e, n), e.omit = (n) => Hn(e, n), e.partial = (...n) => Qn(un, e, n[0]), e.required = (...n) => et(nn, e, n[0]);
});
function po(e, u) {
  const n = {
    type: "object",
...
const Je = /* @__PURE__ */ d("ZodEnum", (e, u) => {
...
const un = /* @__PURE__ */ d("ZodOptional", (e, u) => {
  Wu.init(e, u), N.init(e, u), e._zod.processJSONSchema = (n, t, r) => Qu(e, n, t, r), e.unwrap = () => e._zod.def.innerType;
});
function Eu(e) {
  return new un({
    type: "optional",
...
const go = /* @__PURE__ */ d("ZodExactOptional", (e, u) => {
  Gt.init(e, u), N.init(e, u), e._zod.processJSONSchema = (n, t, r) => Qu(e, n, t, r), e.unwrap = () => e._zod.def.innerType;
});
function Bo(e) {
  return new go({
    type: "optional",
...
function wo(e, u) {
  return new vo({
    ...
    innerType: e,
    get defaultValue() {
      return typeof u == "function" ? u() : Pu(u);
    }
  });
}
...
const nn = /* @__PURE__ */ d("ZodNonOptional", (e, u) => {
...
const jo = po({
  ...
  const e = an.join(hn, "waverider.tmp"), u = [Cn, ":", Fn].join(""), n = cn("scp", [u, e]);
  n.stdout.on("data", (t) => {
    console.log(t.toString());
  }), n.stderr.on("data", (t) => {
    console.error(t.toString());
  }), n.on("close", async (t) => {
    t === 0 ? await Bu.rename(e, vu, (r) => {
      if (r) throw r;
      console.log("main.ts >> Data transfer complete");
    }) : console.error(`main.ts >> scp failed with code -- ${t}`);
  });
}, xo = async () => {
  const e = Bu.readFileSync(vu, "utf-8"), u = JSON.parse(e);
  return Ro(u);
};
function Jo() {
  le.handle("get-drive-data", xo);
}
const Me = process.env.VITE_DEV_SERVER_URL, Xo = J.join(pe, "dist-electron"), Mo = J.join(pe, "dist");
process.env.VITE_PUBLIC = Me ? J.join(pe, "public") : Mo;
let V;
function tn() {
  V = new gu({
    webPreferences: {
  ...
  }), Me ? V.loadURL(Me) : (console.log(q.getAppPath()), V.loadFile("./dist/index.html"));
}
q.on("window-all-closed", () => {
  process.platform !== "darwin" && (q.quit(), V = null);
});
q.on("activate", () => {
  gu.getAllWindows().length === 0 && tn();
});
q.whenReady().then(() => {
  tn(), jn(Vo), Jn(), _u(), setInterval(_u, 1e3 * 60 * 60), Jo(), le.handle("get-height-options", async () => Bn), le.handle("get-period-options", async () => yn);
});
function Vo(e, ...u) {
  V && !V.isDestroyed() && V.webContents.send(e, ...u);
}
process.on("SIGINT", Ge);
process.on("SIGTERM", Ge);
q.on("before-quit", Ge);
export {
  Xo as MAIN_DIST,
  Mo as RENDERER_DIST,
